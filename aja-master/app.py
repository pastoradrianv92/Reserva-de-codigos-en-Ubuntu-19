...
def chart_correction(filename):
    archivo = xl.load_workbook(filename)
    sheet = archivo['Sheet1']
    filas = sheet.max_row
    for celda in range(2,filas+1):
        valor_original = sheet.cell(celda,3)
        cantidad_valor_original = valor_original.value
        valor_corregido = cantidad_valor_original*0.9
        nueva_posicion = sheet.cell(celda,4)
        nueva_posicion.value = valor_corregido

    datos = Reference(sheet,
              min_row=2,
              max_row=4,
              min_col=3,
              max_col=3)
    ...
    archivo.save('transactions2.xlsx')


chart_correction('transactions.xlsx')
